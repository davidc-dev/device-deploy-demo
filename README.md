...
> Legacy raw manifests (`k8s-backend.yaml`, `k8s-frontend.yaml`) remain for reference but the Helm chart keeps the two workloads in sync and is easier to configure on OpenShift (including Route support with an internal API proxy).
...
- Route domains come from the backend automatically; click **Refresh** to load data using backend ArgoCD credentials.
...
| Dashboard fetch | `/argocd/apps` proxies to ArgoCD using the same server-side credentials, strips each `item` to essentials (name, health, sync, repo URL, destination) and enriches it with the actual Route host pulled from the cluster. Frontend parses application names into device name/ID and surfaces Repo / ArgoCD / Dev Spaces / Route links without manual inputs. |
